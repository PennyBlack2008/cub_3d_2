...
void		draw_a_wall(int i, t_ray *r, t_win *w)
{
	...
	dist_to_wall = hypot(r->hit.x - w->player.plot.x, r->hit.y - w->player.plot.y) * fabs(cos(r->ang - w->player.ang));
	// dist_to_wall = hypot(r->hit.x - w->player.plot.x, r->hit.y - w->player.plot.y);
	// dist_to_wall = hypot(r->x - w->player.plot.x, r->y - w->player.plot.y);

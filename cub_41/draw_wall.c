...
void			draw_wall_sprite(int i, t_ray *r, t_win *w, double *temp, int *temp_i)
{
	double		dist_to_wall;
	double		dist_to_spr;
	...
	{
		// if (dist_to_wall > dist_to_spr)
		// {
		// 	draw_a_wall(i, r, w);
		// 	draw_a_sprite(i, r, w);
		// }
		// else if (dist_to_wall < dist_to_spr)
		// {
		// 	draw_a_sprite(i, r, w);
		// 	draw_a_wall(i, r, w);
		// }
		if (*temp_i != 0)
			draw_a_sprite(*temp_i, r, w);
		*temp_i = i; // 과거 스프라이트의 위치 인덱스 저장
		*temp = dist_to_spr; // 과거 플레이어로부터 스프라이트의 거리를 저장
	}
	// else
	
	// 저장한 sprite의 위치가 벽보다 더 멀리 있게 될 때 출력한다.
	if (dist_to_wall < *temp)
	{
		draw_a_sprite(*temp_i, r, w);
	}
}
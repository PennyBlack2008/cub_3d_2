...
int					rotate_right(t_win *w)
{
	...
	new_angle = w->player.ang - 70 * M_PI / 180;
...
int					move_forward(t_win *w)
{
	...
	while (x < w->player.width / 2)
	{
		...
		while (y < w->player.width / 2)
		{
			...
			plot_player.x = plot.x + w->player.x + 70 * cos(w->player.ang * -1);
			plot_player.y = plot.y + w->player.y - 70 * sin(w->player.ang * -1);
			if (plot_player.x >= 0 && plot_player.y >= 0)
			{
				if (is_wall(plot_player.x, plot_player.y, w) == WALL)
					return (WALL);
	...
	w->player.x += 70 * cos(w->player.ang * -1);
	w->player.y -= 70 * sin(w->player.ang * -1);
	return (NOT_WALL);
}
...
int					move_left(t_win *w)
{
	...
	while (x < w->player.width / 2)
	{
		...
		while (y < w->player.width / 2)
		{
			plot.x = x * cos(w->player.ang * -1) + y * sin(w->player.ang * -1);
			plot.y = x * sin(w->player.ang * -1) * -1 + y * cos(w->player.ang * -1);
			plot_player.x = plot.x + w->player.x - 70 * cos(M_PI_2 - (w->player.ang * -1));
			plot_player.y = plot.y + w->player.y - 70 * sin(M_PI_2 - (w->player.ang * -1));;
			if (plot_player.x >= 0 && plot_player.y >= 0)
			{
				if (is_wall(plot_player.x, plot_player.y, w) == WALL)
					return (WALL);
	...
	w->player.x -= 70 * cos(M_PI_2 - (w->player.ang * -1));
	w->player.y -= 70 * sin(M_PI_2 - (w->player.ang * -1));
	return (NOT_WALL);
}

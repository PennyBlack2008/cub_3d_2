...
static void			draw_part_down(int i, int l, double o_pjtd_height, 
		double pjtd_height, double pjtd_width, double scale_w, double scale_h,
	   	t_win *w)
{
	int				color;
	int				j;
	...
	{
		color = get_color_spr(pjtd_width / 2 + l, o_pjtd_height / 2 + j, 
				scale_w, scale_h, w, 4);
		if (color != 0)
		...
		t_win *w)
{
	...
	while (k > 0 && i + l > 0)
	{
		color = get_color_spr(pjtd_width / 2 + l, o_pjtd_height / 2 - k, 
				scale_w, scale_h, w, 4);
		if (color != 0)
		...
		double scale_w, double scale_h, t_ray *r, t_win *w)
{
	...
	{
		draw_part_down(i, l, o_pjtd_height, pjtd_height, pjtd_width, scale_w, scale_h, w);
		draw_part_up(i, l, o_pjtd_height, pjtd_height, pjtd_width, scale_w, scale_h, w);
		if (i + l > w->R_width)
			break ;
		l++;
	}
...
void				draw_a_sprite(int i, t_ray *r, t_win *w)
{
	...
	dist_to_spr = hypot(r[i].spr_hit.x - w->player.x, r[i].spr_hit.y 
			- w->player.y) * fabs(cos(r[i].ang - w->player.ang));
	...
	scale_h = o_pjtd_height / (double)64.0;
	scale_w = o_pjtd_width / (double)64.0;

...
/*   Updated: 2020/11/11 17:09:20 by jikang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

int					get_color_spr(double x, double y, t_sprite_var *var, t_win *w)
{
	int				color;
	double			px;
	double			py;
...
static void			draw_part_down(int i, int l, t_sprite_var *var, t_win *w)
{
	...
	{
		color = get_color_spr(var->o_pjtd_width / 2 + l, var->o_pjtd_height / 2 + j, var, w);
...
static void			draw_part_up(int i, int l, t_sprite_var *var, t_win *w)
{
	...
	while (k > 0)
	{
		color = get_color_spr(var->o_pjtd_width / 2 + l, var->o_pjtd_height / 2 - k, var, w);
		if (color != 0)
...
void				draw_sprite_pixels(int i, t_sprite_var *var, t_ray *r, t_win *w)
{
	int				l;
...
void				draw_a_sprite(int i, t_ray *r, t_win *w)
{
	...
	if (var.dist_to_spr < 30)
		var.dist_to_spr = 30;
	var.pjtd_height = w->wall.height * w->player.projected_plane / var.dist_to_spr;
